...
// Write a function,  std::cout <<positioningPlants, that takes in a 2D array with dimensions n * m.
...
int positioningPlants(std::vector<std::vector<int>>);
int _positioningPlants(std::vector<std::vector<int>>, std::unordered_map<std::string, int> &, int, int);

int positioningPlants(std::vector<std::vector<int>> costs) {
    // todo
    std::unordered_map<std::string, int> hash;
    return _positioningPlants(costs, hash, 0, -1);
}

int _positioningPlants(std::vector<std::vector<int>> costs, std::unordered_map<std::string, int> &hash, int pos, int lastPos) {
...
int main() {
    ...
    std::cout << positioningPlants(std::vector<std::vector<int>>{
                     {4, 3, 7},
                     {6, 1, 9},
                     {2, 5, 3}})
              << std::endl; // -> 7, by doing 4 + 1 + 2.
    std::cout << positioningPlants(std::vector<std::vector<int>>{
                     {12, 14, 5},
                     {6, 3, 2}})
              << std::endl; // -> 8
